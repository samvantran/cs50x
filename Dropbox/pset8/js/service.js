...
function pickup()
{   
    // calculate shuttle's distance, d, from some point (lat, lng)
    for (var i = 0; i < PASSENGERS.length; i++)
    {
        ...
        if (d <= 15.0)
        {
            for (var j = 0; j < shuttle.seats.length; j++)
            {
                ...
                {
                    alert("Seats on shuttle are full! Please drop off current passengers first");
                    break;
                }
            }
        } 
    }
    alert("There are no passengers within 15 meters to pick up!");
}
